...
## Introduction
This role installs (and configures) Grant in the specified host. The role can also set a custom time to trigger the report generation (with cron jobs).
...
- `grant_time` (dictionary, optional): If present, a cron job will be executed at the specified time. Default: `weekday: "0"`, `hour: "3"`, `minute: "0"` (every Sunday at 3 AM). Available fields:
```yaml
  month: "0"
  day: "0"
  weekday: "0"
